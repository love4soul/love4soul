...
umstelJS.loadFile = function(i, j, k) {
    ...
    function s(e, f) {
        ...
        var g = D('../version?t=' + (new Date).getTime());
    ...
    function v() {
        for (var a = 0; a < l.requestedStyles.length; a++) {
            G = l.requestedStyles[a];
            if (G != null && G.status == q) {
                u(G)
            }
        }
    }
